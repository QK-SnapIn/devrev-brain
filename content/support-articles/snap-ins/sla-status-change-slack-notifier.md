...
summary: "Get alerted on your ticket\"s SLAs."
---

# SLA status change Slack notifier

Get alerted on your [[entities/ticket|ticket]]'s [[features/slas|SLAs]]. This snap-in sends a notification through Slack to user configured channels, tagging the ticket's owner and subscribers, when a ticket's resolution time [[features/slas|SLA]] changes into the *Warning* or *Breached* stage.
...
7. In the DevRev app, setup the connection in **Settings** > **Snap-ins** > **Connections** on top.
   ...
   * [[features/search|Search]] and choose an existing connection or create a new one by clicking **+ Connection**.
...
1. In the **Configuration** tab, the first input field to set is *filters*. Here you can declare for which [[features/tickets|tickets]] to track the SLA status and to which channels to send notifications.

   Set the ticket subtype, severity, and [[entities/part|part]]; the support heads you would like to tag on the message (the ticket owner gets tagged automatically); and the target Slack channel. The channel's ID can be found by going to the channel details. Refer to the placeholder value on the input to see an example of how this looks.
...
## Source
- DevRev support [[entities/article|article]] [SLA status change Slack notifier](https://support.devrev.ai/en-US/devrev/article/RdtItDkh) (ART-21951)
