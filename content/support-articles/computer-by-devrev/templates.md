---
title: Templates
devrev_id: ART-21856
parent_directory: Computer by DevRev
translation_group: okzRmK_a
modified_date: "2025-12-10T06:56:24.593Z"
source_url: "https://support.devrev.ai/en-US/devrev/article/okzRmK_a"
tags: []
top_category: Computer by DevRev
wiki_match: features/templates
match_score: 1.0
last_updated: 2026-05-11
related: ['features/templates']
---

# Templates

DevRev enables the creation and use of templates to accelerate record creation and help creators share information effectively. Currently, DevRev supports templates for issues, tickets, and articles at the workspace level.

## Manage templates

To create, edit, use, and delete templates, go to [**Settings** > **Templates**](https://app.devrev.ai/?setting=record-template).

## Creating templates

1. Click **+ Templates**.
2. Enter a **Name** (required) and an optional **Description** for the template.
3. Select the **Object Type** (*Article*, *Issues*, *Tickets*) under which the template will be categorized.
4. Click **Next**.
5. Preset fields based on the selected template type are displayed. You can customize these by adding additional attributes if necessary.

## Editing templates

To edit or delete a template, navigate to [**Settings** > **Templates**](https://app.devrev.ai/?setting=record-template). Under the **Action** column, select **Edit** or **Delete**.

## Using templates for tickets and issues

1. Click **+** in the record creation panel at the top of the DevRev app main page, and select either **Ticket** or **Issue**.
2. Click the template icon next to the **Select Subtype** drop-down menu and select a template. The selected template is applied to the object.

## Using article templates

1. Under [**Settings > Knowledge Base**](https://app.devrev.ai/?setting=knowledge-base%2Farticles), go to **View Templates**. A modal opens, showing a list and preview of the available templates.
2. Select a template and click **Use Template**. The article opens with the template information applied.

## Related wiki nodes
- [[features/templates]]

## Source
- DevRev support article [Templates](https://support.devrev.ai/en-US/devrev/article/okzRmK_a) (ART-21856)
