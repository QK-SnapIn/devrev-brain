...
category: airsync
source_url: https://developer.devrev.ai/airsync/manifest
last_updated: 2026-05-11
summary: "Start developing your snap-in with a manifest.yaml file."
---
...
The manifest must start with a `version` field:
...
### [Set snap-in functionalities](#set-snap-in-functionalities)
...
## [Establish a connection to the external system](#establish-a-connection-to-the-external-system)
...
### [Configure a keyring](#configure-a-keyring)
...
keyring_types:
  - id: <id of the keyring type>
    ...
    secret_config:
      ...
      # Optional: A jq expression to extract the token from the secret fields.
      # For example, ".token" extracts the value of the "token" field.
      secret_transform: <jq_expression>
...
* `kind`  
  The `kind` option can be either "Secret" or "oauth2". The "Secret" option is intended for storing various tokens, such as a PAT token. Use of OAuth2 is encouraged when possible. More information is available for [secret](https://developer.devrev.ai/snapin-development/references/keyrings/secret-configuration) and [oauth2](https://developer.devrev.ai/snapin-development/references/keyrings/oauth-configuration).
* `is_subdomain`  
...
## Source
- DevRev developer docs: [Manifest](https://developer.devrev.ai/airsync/manifest)
